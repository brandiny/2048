...
class MainGame(tk.Frame):
    ...
    def __init__(self, parent, controller, new_game=True):
        ...
        self.back_button_game.place(
            x=controller.GAME_WIDTH - self.back_button_game.winfo_reqwidth() - 20,
        ...
        self.restart_button.place(
            x=controller.GAME_WIDTH - self.restart_button.winfo_reqwidth()
            - 30 - self.back_button_game.winfo_reqwidth(),
            y=controller.GAME_HEIGHT - self.back_button_game.winfo_reqheight() - 20)
        ...
        if not self.name or self.predicted_value == -1:
            ...
            # preemptively place the game over screen (invisible)
    ...
    def is_game_finished(self):
        ...
        if not self.any_possible_moves_horizontal() and \
            ...
            game_over_description = tk.Label(
                ...
                text='Name: {}\nScore: {}\nDistance from prediction: {}'.format(
                    self.name,
                    str(self.score_value.get()),
                    str(abs(int(self.predicted_value) - int(self.score_value.get())))
                ),
                justify=tk.LEFT,
                anchor=tk.W)
            ...
            self.game_over_frame.place(
                x=self.controller.GAME_WIDTH // 2 - 0.5 * self.game_over_frame.winfo_reqwidth() - 200,
                y=self.controller.GAME_HEIGHT // 2 - 0.5 * self.game_over_frame.winfo_reqheight() - 180)
